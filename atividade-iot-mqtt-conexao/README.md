# Trabalho IoT — MQTT com Mosquitto: de um tópico para uma estação organizada
...
> Você recebe um **ponto de partida** que já funciona: o ESP32 conecta, publica **tudo em um único
> tópico** (temperatura, umidade, LED e os **logs** de Wi-Fi e de TLS) e ouve um tópico de comando.
> **O trabalho é evoluir essa base:** dividir em vários tópicos, acrescentar o tópico do LED,
> organizar os logs e implementar **Deep Sleep**.

Este documento tem três partes: **Entenda** (a teoria — leia primeiro), **O ponto de partida**
(o código que você recebe) e **O trabalho** (o que você deve implementar e entregar).
...
> Como é **público**, todo mundo vê os mesmos tópicos. Use um **prefixo único** (troque `leonam`
> pelo seu nome/matrícula) e não mande nada sensível.
...
```
sis1a/leonam/temp        →  temperatura
sis1a/leonam/umid        →  umidade
sis1a/leonam/comando     →  ordens que voltam para o ESP32
```

Ao **assinar**, você pode usar **curingas**:
- **`+`** troca **um nível**: `sis1a/+/temp` = a temperatura de **todos** os alunos.
- **`#`** troca **o resto**: `sis1a/leonam/#` = **tudo** do aluno "leonam".
...
```bash
mosquitto_sub -h test.mosquitto.org -t "sis1a/leonam/#" -v                    # LER
mosquitto_pub -h test.mosquitto.org -t "sis1a/leonam/comando" -m "limite:25"  # ESCREVER
```
